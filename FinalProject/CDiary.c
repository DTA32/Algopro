...
int main(){
    ...
    while(ch != 4){
        ...
        if(ch == 1){
        	bikin_diary();
        	printf("Diary berhasil ditambahkan! Press ENTER to continue."); getchar(); getchar();
        }
        else if(ch == 2){
				show_dia();
				showall();
			...
			printf("Diary ke ");
			scanf("%d", &hapus);
			printf("Diary berhasil dihapus! Press ENTER to continue."); getchar(); getchar();
			dele(hapus);
        }
    }
}
